...
INSERT INTO queries VALUES("setgrent", "SELECT gid, groupname, passwd FROM groups");
INSERT INTO queries VALUES("getgrnam_r", "SELECT gid, passwd FROM groups WHERE groupname = ?");
INSERT INTO queries VALUES("getgrgid_r", "SELECT groupname, passwd FROM groups WHERE gid = ?");

INSERT INTO queries VALUES("initgroups_dyn", "SELECT ug.gid FROM user_group ug INNER JOIN passwd p ON p.uid = ug.uid WHERE p.username = ? AND ug.gid != ?");
INSERT INTO queries VALUES("get_users", "SELECT username FROM passwd u INNER JOIN user_group ug ON ug.uid = u.uid WHERE ug.gid = ?");
