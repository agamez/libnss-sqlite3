CREATE TABLE passwd(uid INTEGER PRIMARY KEY, username TEXT NOT NULL, passwd TEXT NOT NULL default 'x', gid INTEGER, gecos TEXT NOT NULL default ',,,', homedir TEXT NOT NULL, shell TEXT NOT NULL);
...
INSERT INTO nss_queries VALUES("setpwent", "SELECT uid, username, passwd, gid, gecos, homedir, shell FROM passwd;");
INSERT INTO nss_queries VALUES("getpwnam_r","SELECT uid, passwd, gid, gecos, homedir, shell FROM passwd WHERE username = ?");
INSERT INTO nss_queries VALUES("getpwuid_r","SELECT username, passwd, gid, gecos, homedir, shell FROM passwd WHERE uid = ?;");
